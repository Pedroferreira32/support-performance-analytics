...
import { competencia, monthlyClosures } from "@/data/support-data-runtime";
...
const allValues = data.flatMap((item) => [item.indice, item.mediana, competencia.meta]);
const domainMin = Math.max(0, Math.floor((Math.min(...allValues) - 3) / 5) * 5);
const domainMax = Math.ceil((Math.max(...allValues) + 3) / 5) * 5;
